...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Servicios.HttpApi.Host` and `Servicios.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 754b536b-9c92-4c1f-94fe-69751197089f
```

> `754b536b-9c92-4c1f-94fe-69751197089f` is the password of the certificate, you can change it to any password you want.
...
Run `Servicios.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `Servicios.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `Servicios.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
